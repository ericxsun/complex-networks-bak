function trees = spanning_tree(edge, directed)
%SPANNING_TREE find all the connected subgraph of a given graph  using an
%algorithm similar to things in Disjoint-set data structure.
%
%Syntax: 
% trees = SPANNING_TREE(edge, directed)
%
...
%           directed one.
%
%    trees: all the subgraph, trees{i} records the i-th connected subgraph.
%
%Example:
%
%Ref:
...
for i = 1 : n_gt0
	nodes = grp{gt0(i)};
	trees{i} = edge(ismember(edge(:, 1), nodes) | ismember(edge(:, 2), nodes), :);
end